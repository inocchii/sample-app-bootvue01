...
export default {
   ...
   */
    console.log("ThemesVue tabIndex kept in this.$store.state.TABINDEXES :"+this.$store.state.TABINDEXES['ThemesVue']);
    console.log("ThemesVue can access this.state.TABINDEXES? :"+this.state.TABINDEXES['ThemesVue']);
  },
  data() {
    return {
      tabIndex: 0,
  ...
  watch: {
    tabIndex: function() {
      ...
      // vuex
      this.$store.commit('setTabIndex',['ThemesVue',this.tabIndex]);
      console.log("ThemesVue tabIndex kept to vuex TABINDEXES[ThemesVue] :"+this.$store.state.TABINDEXES['ThemesVue']);
      // mixin
      ...
      console.log("ThemesVue tabIndex kept not in mixin but local):"+this.TAB_INDEX);
    },
  },
};
</script>
